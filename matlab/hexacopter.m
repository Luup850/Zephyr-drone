%% Noter
% Lead led er lagt i measurement. Det er mere modstandsdygtig over for
% støj. Da forstyrrelser kommer efter controller.
...
debugPlot = 0; % set to 0 for no plot
...
Gh1bv = Gh1b * tf([1 0], [1]); % Vel out
Gh1b_d = c2d(Gh1b, Ts, 'zoh'); % 1/120, update rate on optiTrack
Gh1bv_d = c2d(Gh1bv, Ts, 'zoh');
Gh1 = minreal(Gpipo(2))
% bode sammenligning
figure(1002)
hold off
...
% poles:  s = 0, -13, -0.18
%% MC: Design height vel controller
%h1vgm = 60;
%h1val = 0.1;
%h1vNi = 4;
%h1vti = 0;
% Differentiate to get vel.
%Gh1av = Gh1a * tf([1 0], [1]);
%[h1vw, h1vkp, h1vtd] = findpd(Gh1av, h1vgm, h1val)
%[h1vw, h1vkp, h1vti, h1vtd] = findpid(Gh1av, h1vgm, h1vNi, h1val)
% Gh1av open loop. Used for tuning next controller
%Gh1avol = h1vkp * tf([h1vtd 1], [h1vtd*h1val 1]) * Gh1a * tf([h1vti 1], [h1vti 0]); % With I term
...
%[h1w, h1kp, h1td] = findpd(Gh1avol, h1gm, h1al)
%[h1w, h1kp] = findp(Gh1avol, h1gm)
% result:
...
ral = 0.5;
rNi = 0;
display("Roll velocity Regulator") %Marcus
...
ragm = 80;
raNi = 0;
display("Roll Angle Regulator") %Marcus
...
% estimate transfer function
Gxv = Gpacl * tf([1],[1 0]) * trustHoover;%(180/pi); % MC: trustHoover shouldnt be here.
